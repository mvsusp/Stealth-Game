...
public class LastPlayerSighting : MonoBehaviour {
    ...
    void Awake()
    {
        ...
        var sirenGameObjets = GameObject.FindGameObjectsWithTag(Tags.siren);
        sirens = new AudioSource[sirenGameObjets.Length];

        for (int i = 0; i < sirens.Length; i++)
        {
            sirens[i] = sirenGameObjets[i].GetComponent<AudioSource>();
        }
    }
    ...
    void MusicFading()
    {
        ...
        else
        {
            ...
            panicAudio.volume = Mathf.Lerp(panicAudio.volume, 0, musicFadeSpeed * Time.deltaTime);
        }
    }
}
